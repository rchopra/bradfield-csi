...
typedef struct lsstats {
  blkcnt_t total_blocks;
  off_t total_size;
} LSSTATS;

// Function prototypes
unsigned char parse_args(char *path, int argc, char *argv[]);
int collect_contents(DIR *dirp, LSENT *entries, LSSTATS *summary, unsigned char flags);
void print_summary(LSSTATS *summary, unsigned char flags);
void print_entry(LSENT *entry, unsigned char flags);
int compare_by_size(const void *first, const void *second);

int main(int argc, char *argv[]) {
  char path[MAX_PATH_LEN];
  unsigned char flags = parse_args(path, argc, argv);
  ...
  int num_entries = collect_contents(dirp, entries, &summary, flags);
...
int compare_by_size(const void *first, const void *second) {
  ...
  // A tip learned from StackOverflow, this avoids problems with
  // overflow from doing something like: return ssize - fsize;
